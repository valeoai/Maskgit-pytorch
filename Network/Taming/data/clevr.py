...
from Network.Taming.data.base import ImagePaths
import numpy as np
...
class CLEVR(VisionDataset):
    ...
    def __init__(
        ...
        self._labels: Dict[str, List[Optional[int]]] 
        ...
        if self._split != "test":
            ...
            self._labels = {"class_label": np.array(self._labels)}
        else:
            self._labels = [None] * len(self._image_files)
            self._labels = {"class_label": np.array(self._labels)}

        self.size = size
        self.data = ImagePaths(self._image_files, 
                               labels=self._labels,
                               size=size)
    ...
    def __getitem__(self, i: int) -> Tuple[Any, Any]:
        ...
        for k, v in self.data[i].items():
            sample[k] = str(v) if isinstance(v, pathlib.PosixPath) else v
        return sample['image'].transpose(2, 0, 1), sample['class_label']
